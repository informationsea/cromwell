...
case class CommandLineArguments(command: Option[Command] = None,
  ...
  def validateSubmission(logger: Logger): ErrorOr[ValidSubmission] = {
    ...
    val workflowSourceAndUrl: ErrorOr[WorkflowSourceOrUrl] = DefaultPathBuilder.build(workflowSource.get) match {
      ...
      case Failure(_) =>
        PartialWorkflowSources
          .validateWorkflowUrl(workflowSource.get)
          .map(validUrl => WorkflowSourceOrUrl(None, Option(validUrl)))
    }

    val inputsJson: ErrorOr[String] = readOptionContent("Workflow inputs", workflowInputs)

    import common.validation.ErrorOr.ShortCircuitingFlatMap
    val optionsJson = readOptionContent("Workflow options", workflowOptions).flatMap {
      WorkflowOptions.fromJsonString(_).toErrorOr
    }
    ...
    (workflowSourceAndUrl, inputsJson, optionsJson, labelsJson) mapN { case (srcOrUrl, i, o, l) =>
