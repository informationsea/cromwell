...
class CommandLineParser extends scopt.OptionParser[CommandLineArguments]("java -jar /path/to/cromwell.jar") {
  ...
  private def commonSubmissionArguments = List(
    arg[String]("workflow-source")
      .text("Workflow source file or workflow url.")
      .required()
      .action((s, c) => c.copy(workflowSource = Option(s))),
    opt[String]("workflow-root").text("Workflow root.").action((s, c) => c.copy(workflowRoot = Option(s))),
    opt[String]('i', "inputs")
      .text("Workflow inputs file.")
      .action((s, c) => c.copy(workflowInputs = Option(DefaultPathBuilder.get(s)))),
    opt[String]('o', "options")
      .text("Workflow options file.")
      .action((s, c) => c.copy(workflowOptions = Option(DefaultPathBuilder.get(s)))),
    opt[String]('t', "type").text("Workflow type.").action((s, c) => c.copy(workflowType = Option(s))),
    opt[String]('v', "type-version")
      .text("Workflow type version.")
      .action((s, c) => c.copy(workflowTypeVersion = Option(s))),
    opt[String]('l', "labels")
      .text("Workflow labels file.")
      .action((s, c) => c.copy(workflowLabels = Option(DefaultPathBuilder.get(s)))),
    opt[String]('p', "imports")
      .text("A zip file to search for workflow imports.")
      .action((s, c) => c.copy(imports = Option(DefaultPathBuilder.get(s))))
  )
  ...
  cmd("server")
    .action((_, c) => c.copy(command = Option(Server)))
    .text(
      "Starts a web server on port 8000.  See the web server documentation for more details about the API endpoints."
    )
  ...
  cmd("run")
    ...
    .children(
      commonSubmissionArguments ++ List(
        opt[String]('m', "metadata-output")
          .text("An optional JSON file path to output metadata.")
          .action((s, c) => c.copy(metadataOutput = Option(DefaultPathBuilder.get(s))))
      ): _*
    )
